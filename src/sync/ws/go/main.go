...
func main() {
	...
	pflag.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, strings.Join([]string{"",
			"Examples:",
			"  together-code-ws-server",
			"  together-code-ws-server -p 9000",
			"  together-code-ws-server --port 9000 --hostname 0.0.0.0",
		}, "\n"))
	}
	pflag.Parse()
...
func handleConnection(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" && r.URL.Path == "/" {
		...
		msg := fmt.Sprintf("Together Code WebSocket Signaling Server. %d active room(s).", roomCount)
